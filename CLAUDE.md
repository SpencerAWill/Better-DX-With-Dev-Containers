...
Polyglot monorepo demonstrating dev containers for an online ordering platform. Multi-language (TypeScript + C#) with sidecar containers (PostgreSQL, Azure Service Bus emulator, Azurite, Azure Cosmos DB emulator, Azure Event Hubs emulator) emulating production dependencies.
...
- `.devcontainer/` — Dev container config (Dockerfile + Docker Compose with PostgreSQL, Service Bus emulator, Azurite, Cosmos DB emulator & Event Hubs emulator sidecars)
...
- Ports: 5173 (Vite), 5258 (HTTP API), 7130 (HTTPS API), 7071 (Azure Functions), 5432 (PostgreSQL), 8081 (Cosmos DB), 9092 (Event Hubs Kafka), 10000-10002 (Azurite)
